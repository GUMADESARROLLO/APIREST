...
class Vistas{
    ...
    public static function Pts(){
        $link = @mysql_connect('localhost', 'root', 'a7m1425.')or die('No se pudo conectar: ' . mysql_error());
        mysql_select_db('dbvineta') or die('No se pudo seleccionar la base de datos');
        $consulta= 'call pc_line_factura_puntos ';
        $result = mysql_query($consulta) or die('Consulta fallida: ' . mysql_error());
        $json = array();
        $i=0;
        while($line = mysql_fetch_array($result, MYSQL_ASSOC)){
            $json[$i]["CLIENTE"] = $line['CLIENTE'];
            $json[$i]["FACTURAS"] = $line['pts'];
            $i++;
        }
        echo json_encode($json);
    }
    public static function MTCL($ID){
        $obj = new Vistas;
        $conn = $obj -> open_database_connectionSQL();  
        $tsql = "SELECT top 10 * FROM dbo.APKSaldo_clientes_umk where VENDEDOR='".$ID."' ORDER BY NOMBRE ";
    ...
    public static function Login($User,$Pass){
        $obj = new Vistas;
        $link = $obj ->open_database_connectionMYSQL();
        $consulta= "SELECT * FROM usuario WHERE CodVendedor='".$User."' and Password='".$Pass."'"; 
